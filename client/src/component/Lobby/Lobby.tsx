...
const Lobby: React.FC<ILobbyProps> = ({ epages }) => {
    ...
    return (
        <div className="container-Lobby">
          ...
          <div onClick={() => setPanel(PANEL.ADDAFRIEND1)} className="friend"></div>
          </div>
          ...
          <div onClick={() => setPanel(PANEL.ADDAFRIEND2)} className="friend-2"></div>
          </div>
          
          <button onClick={() => epages(EPAGES.GAME)} className="play">ИГРАТЬ</button>
        </div>
    );
};
